...
| **C** long prefill | synthetic · in≈4000, out=32, conc=4 | **12.119**<br>±0.116 | **21.530**<br>±1.37<br>**(+77.7%)** | **14.939**<br>±0.123<br>**(+23.26%)** | 12.869<br>±0.182<br>(+6.19%) | **20.413**<br>±1.9<br>(+68.4%) ² | **22.879**<br>±1.07<br>**(+88.8%)** | **16.392**<br>±0.200<br>**(+35.26%)** | **21.717**<br>±0.5<br>(+79.2%) ² |
...
### 1.1 Throughput and latency are the same measurement in five of the six regimes

The tool-agent row looks like the kernel work stops paying on real traffic. It does not.
`bench_serving` defaults `--request-rate` to infinity, so in five of the six workloads every
request is released at t=0 and only `--max-concurrency` gates them: a slot frees, the next
request enters, and the server runs saturated. The mooncake replay is the exception —
`get_mooncake_request_over_time` sleeps until each record's own trace timestamp, so arrivals
are set by the trace and are completely independent of how fast the server answers.

Little's Law separates the two with no new measurement. The average number of requests in
flight is throughput × latency; a saturated closed loop must sit at its concurrency cap.

| Regime | cap | throughput | E2E mean | **N in flight** | **utilisation** |
|---|---:|---:|---:|---:|---:|
| A low-batch decode | 1 | 1.686 | 591.5 ms | 1.00 | **100%** |
| C long prefill | 4 | 12.119 | 321.4 ms | 3.90 | **97%** |
| D medium balanced | 8 | 6.900 | 1153.9 ms | 7.96 | **100%** |
| B concurrent decode | 32 | 21.661 | 1463.5 ms | 31.70 | **99%** |
| E shared prefix | 64 | 14.220 | 3923.5 ms | 55.79 | **87%** |
| **F tool agent** | 64 | 5.265 | 901.7 ms | **4.75** | **7%** |

**F uses 4.75 of the 64 concurrency slots it is allowed.** The server is idle waiting for the
trace 93% of the time, so making it faster cannot retire more requests per second — there
are none waiting.

Which means the throughput column has been reporting a latency result all along. With N
pinned, throughput = N / latency, so a 6% latency cut *is* a 6% throughput gain:

| Regime | E2E mean | TTFT p50 | TPOT p50 | thr **predicted** from latency | thr **measured** |
|---|---:|---:|---:|---:|---:|
| A low-batch decode | −6.29% | −5.73% | −6.32% | +6.71% | **+6.70%** |
| C long prefill | −6.27% | −6.19% | −6.45% | +6.69% | **+6.18%** |
| D medium balanced | −7.67% | −5.80% | −7.84% | +8.30% | **+8.29%** |
| B concurrent decode | −6.42% | −7.61% | −6.26% | +6.87% | **+6.72%** |
| E shared prefix | −6.62% | −7.43% | −7.19% | +7.09% | **+7.24%** |
| **F tool agent** | **−6.04%** | **−7.91%** | −4.47% | +6.43% | **+0.40%** ← N moved |

The prediction lands within half a point in five regimes. In F it is out by six points,
because that is the one workload where N is free to move — and it moved into idle time
instead of throughput.

> **The kernel rewrite cuts end-to-end latency by 6.0–7.7% on every workload in the suite,
> including the real trace.** There is no weak regime. There is one regime where the
> throughput metric cannot see the result, and it happens to be the only one whose arrival
> pattern was not chosen by us. On a production trace, latency is also the number that has
> an SLO attached to it.

Generated by `scripts/lfm_fusion/exp3_littles_law.py`.

### 1.2 What the completed matrix says across regimes
...
prompt), +23.26% on C (in≈4000).

### 1.3 The L1 ceilings are not throughput-for-latency trades
