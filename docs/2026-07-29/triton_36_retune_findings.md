...
## 3b. 机制：3.6 到底改了什么？（一个被自己数据推翻的假设）

### 先说结论

**Triton 3.6 改进的是 `BLOCK_SIZE_K=32` 这个区域的代码生成，而那正是 sglang 默认配置所在的位置，也正是我们的搜索空间从未覆盖的位置。**

### 被推翻的假设

我最初的解释是「默认配置不指定 `num_warps`/`num_stages`，3.6 自动选得更好」。
**这是错的。** 直接读两个版本的 Triton 源码（`CUDAOptions` 的 dataclass 默认值）：
...
**完全相同。** 而且 `fused_moe_kernel` 是纯 `@triton.jit`（无 `@triton.autotune`），
sglang 用 `**config` 直接 splat 进 launch（`fused_moe_triton_kernels.py:884`）。

所以默认路径在两个版本上的启动参数**逐字段相同**：
`BM=64, BN=64, BK=32, GM=8, num_warps=4, num_stages=3`。

**同样的参数，3.6 快 1.4–1.7×** —— 那只能是编译出来的机器码不同。

### 定位：把 800+ 个 config 逐一配对

把两个版本的 sweep 结果按 (BM, BN, BK, GM, warps, stages) 精确配对，
n=822（M=96）/ 894（M=3072）：

| BLOCK_SIZE_K | M=96 | M=1536 | M=3072 |
|---|---:|---:|---:|
| 256 | 1.011× | 1.002× | 1.008× |
| 128 | 1.012× | 1.000× | 1.030× |
| 64 | 1.011× | 1.000× | 1.038× |
| **32（默认配置，我们没扫）** | **1.409×** | **1.606×** | **1.699×** |

**我们扫过的每一个配置，3.6 都只快 0–4%。只有默认配置快 40–70%。**

分布统计也印证这点 —— 全候选的 best/p25/median/p75/worst 在两版上几乎重合（差 2–4%），
**3.6 并没有让 kernel 普遍变快**。

### 为什么是 BK=32（这部分是推断，未验证）

`BK` 是 K 维的分块大小，LFM2.5 的 `K=2048`：

- `BK=32` → K 循环 **64 次**迭代，每次搬很小的 tile
- `BK=256` → 只要 8 次

迭代多、每次数据少，正是**软件流水线和访存调度最吃紧**的场景 —— 延迟藏不住、循环开销摊不薄。

注意 1.04× → 1.70× 是**跳变而非渐变**（BK 从 64 降到 32 只差一档，加速却从 1.04 跳到 1.70），
说明 BK=32 上存在某种质变，不是趋势的自然延伸。

**待验证**：固定 `BM=64, BN=64, GM=8`，只扫 `BK ∈ {16, 32, 64}` × `warps` × `stages`，
在两版上对比。这能确认拐点是否恰在 BK=32，以及是否与 warps/stages 有交互。

### 这让 PR 的性质更清楚

> 我们的 tuning 在 3.5.1 上值 1.43×，本质是**绕开了默认配置踩中的那个编译器弱点**
> （改用 BK=64–256 而不是 32）。3.6 把弱点修在了编译器层，所以绕不绕开都一样快。

**我们和 Triton 团队修的是同一个问题，只是层次不同** —— 我们在配置层绕过去，他们在代码生成层解决掉。

### 方法论副产品：我们的搜索空间有盲区

`rk_lib.build_search_space` 的 `BLOCK_SIZE_K` 最小值是 **64**，
而 sglang 默认配置用的是 **32**。也就是说：

> **我们一直在拿一个不包含 baseline 的搜索空间去"打败 baseline"。**

这次侥幸没出错（我们找到的解确实更快），但它意味着**我们从未测量过 baseline 附近的地形**，
也因此差点错过了这次真正的机制。搜索空间**必须包含 baseline 本身**，否则无法解释自己赢在哪里。
