...
| 端到端 | 见 §6——**必须区分「对 main」和「对 #32670 基线」两个数字** |

**必须先说的一件事**：本工作最初测出的端到端数字是 **1.33–1.41×**（吞吐 +25.7% 到 +37.3%）。
**这个数字不能用**，因为基线是未修的 main，里面还含着 PR #32670 正在修的 rank 守卫缺口。
消融实验显示，那个提升**几乎全部来自 rank 守卫修复**，rope 融合本身的增量小得多。详见 §6。
...
第一版 A/B 只有两臂（main vs fused），结果是 decode **1.33–1.41×**、吞吐 **+25.7% 到 +37.3%**，
...
**见 `results/fx_fusion/e2e_ab_gemma3_ablation.json`。**
初步单次测量（bs=1, in=128, out=8）显示两臂分别是 2.21ms / 2.19ms，
即 rope 融合本身的端到端增量约 **1%**，远小于对 main 的 1.39×。

> **结论：那 1.33–1.41× 几乎全部是 rank 守卫修复的功劳。**
> rope 融合的独立贡献在端到端上很小——preamble 本身只占 decode 步的一小片。
> 微基准的 2.3× 不能外推成端到端 2.3×，这是同一个教训的第四次出现。
...
- 微基准 1.5–2.3×（对 #32670 基线），**端到端增量小得多**（约 1%）
- 最大的端到端收益来自 **rank 守卫修复**，那是 PR #32670 的，不是本次的
...
6. **微基准的加速比不能外推成端到端。** 2.3× 的 preamble 换来约 1% 的端到端，
   因为 preamble 只占 decode 步的一小片。
