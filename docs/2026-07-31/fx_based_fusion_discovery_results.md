...
我们有 11 个模型的人工 profiling 审计基线，其中 5 个也做了 FX 扫描，
...
**「找到几条链」几乎不预测真实机会** —— olmoe 有 13 条/层（第二多），
但审计只有 0.43%；granite 9 条/层，审计 0.23%。
**链多不代表热。**

**而「估计能省多少字节」预测得不错**（ρ=+0.70）——
因为它同时含了链长和张量大小，更接近实际的显存流量。

**给 agent 的直接建议：阶段 1 排序要用字节数，不要用链数。**

⚠️ **n=5，统计上很弱**，只能当作方向性证据。但方向是清楚的，
而且和机制一致：融合省的是显存往返，那自然该按流量而不是按数量排序。

（另有 6 个模型只有人工审计、没有 FX 扫描，其中 lfm25、phi4mini、qwen 系列
可以补上，会让这个相关性更可信。）
