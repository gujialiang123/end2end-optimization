...
1. ~~**可行性验证通过**：写了一个硬件无关的 FX 扫描器，
   ...
   **⚠ 2026-07-31 撤回**：扫描器**没有**独立重现该案例，理由见 §2。
   修正后的表述：扫描器能列出图中所有展开的融合链，但**无法判定其中哪条是框架的缺口**。
...
4. **找到一个 Inductor 融不掉的真实模式**：QK-norm 的两个切片各自 norm 会生成 2 个 kernel，
   合并后在 decode 场景（小 token）可达 **1.96×**。
   ⚠ **更正**：这**不是**扫描器自动找到的——假设来自人工读代码，
   FX 承担的是验证与量化。扫描器结构上找不到它（它把 `slice` 当作透明 op）。
...
## 2. ~~可信度验证：自动化能否重现人工发现~~ → **结论已撤回** ⚠

> **2026-07-31 更正**：本节原结论「扫描器在没有任何先验知识的情况下，独立发现了它」
> **不成立，予以撤回**。下方数据本身无误，但推论错误。
> 完整分析见 `docs/2026-07-31/two_fusion_cases_full_record.md`。
...
>
> **⚠ 撤回。** 两个问题：
> 1. dim=1152 和 dim=256 的链**签名完全相同**，扫描器没有任何机制判定
>    哪一类是缺口——按维度分类那一步是**我用先验知识手工做的**。
> 2. 扫的是 **HF 实现**，而 bug 在 **sglang** 里。HF 没有任何融合 kernel，
>    它所有 RMSNorm 都是展开链，所以「找到展开的链」对 13 条**全部成立**，
>    零区分度。sglang 的缺口是「有 kernel 但 4-D 输入被 `if x.dim()==2` 挡住」。
>
> **正确做法**：扫框架自己的模块、跨 rank 追踪，
> 靠「同一模块内融合与否的**不对称性**」定位。该信号已实测证实存在，
> 但自动枚举的检测器**尚未建成**。
