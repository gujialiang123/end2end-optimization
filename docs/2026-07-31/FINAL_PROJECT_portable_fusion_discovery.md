...
### 结果一：~~自动化能重现人工发现~~ → **已撤回，见下方更正** ⚠

> **2026-07-31 更正**：本节原标题为「自动化能重现人工发现（可信度验证）★」，
> 结论为「扫描器独立发现了它」。**该结论不成立，予以撤回。**
> 完整分析见 `docs/2026-07-31/two_fusion_cases_full_record.md`。

原始数据本身没错——在 `gemma-3-1b` 上扫描确实得到：
...
1. **两类链的签名完全相同**（均为 `pow->mean->add->rsqrt->mul->mul`）。
   扫描器没有任何机制判定「dim=256 是 bug、dim=1152 正常」——
   那一步分类是**我用先验知识手工做的**。
2. **扫的是 HF 实现，而 bug 在 sglang 里**。HF 根本没有融合 kernel，
   它**所有** RMSNorm 都是展开的 eager 链。所以「找到展开的链」对 13 条**全部成立**，
   不构成任何缺口证据。sglang 的 bug 是「**有** kernel 但 4-D 输入被 `if x.dim()==2` 挡住」，
   这是另一回事。

**正确的做法**是扫**框架自己的模块**、跨 rank 追踪，靠「同一模块内融合与否的**不对称性**」定位。
该信号已被实测证实存在（2-D 图含不透明自定义 op / 4-D 图为 10 节点展开链），
但**自动枚举的检测器尚未建成**。
